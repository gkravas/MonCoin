...
- (void)apiQuery:(NSString *)method params:(NSDictionary *)params success:(CryptsyAPISuccessBlock)success error:(CryptsyAPIErrorBlock)error {
    ...
    [newParams setObject:[NSNumber numberWithInt:[NSDate timeIntervalSinceReferenceDate]] forKey:@"time"];
    ...
    NSURLRequest *request = [manager.requestSerializer requestWithMethod:@"GET" URLString:@"https://api.mintpal.com/v2" parameters:sortedParams error:&internalError];
    NSString *query = [[request.URL absoluteString] stringByReplacingOccurrencesOfString:@"https://api.mintpal.com/v2?" withString:@""];
    ...
    [manager POST:@"https://api.mintpal.com/v2" parameters:sortedParams success:^(AFHTTPRequestOperation *operation, id responseObject) {
