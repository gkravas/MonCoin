...
#import "MintpalMonitorViewController.h"

@interface MintpalMonitorViewController ()

@end

@implementation MintpalMonitorViewController {
    BOOL willReload;
}
...
- (void)initAndLoadData {
    ...
    self.api = [[VGMintpalAPI alloc] initWithApiKey:@"123931eeb6baadd53cbdbc755ac81cfd" andSecret:@"6546d5e0692d7d5dfd84c1a25ea9d7"];
...
- (void)getExchangeMarkets {
    ...
    [self.api apiQuery:@"/market/summary/" params:nil success:^(id response) {
        // success
        //NSLog(@"%@", response);
        ...
        for ( NSDictionary *marketData in marketsData ) {
            ...
            model.coinName = marketCoinCode;
            model.exchangeCoinCode = coinCode;
...
- (void) savePreviousTotal:(float)total {
    NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:[NSNumber numberWithFloat:total] forKey:PREV_TOTAL];
    [defaults synchronize];
}

- (float) getPreviousTotal {
    NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
    return [(NSNumber *)[defaults objectForKey:PREV_TOTAL] floatValue];
}

- (void) saveMarkets {
    NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
    NSData *data = [NSKeyedArchiver archivedDataWithRootObject:self.markets];
    [defaults setValue:data forKey:PREV_MARKETS_DATA];
    [defaults synchronize];
}

- (NSDictionary *) getPreviousMarkets {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSData *data = [defaults objectForKey:PREV_MARKETS_DATA];
